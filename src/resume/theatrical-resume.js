export function TheatricalResume() {
    return (
        <div className='theatrical-resume'>
            <link rel="preconnect" href="https://fonts.gstatic.com"/>
            <link href="https://fonts.googleapis.com/css2?family=Comfortaa:wght@700&display=swap" rel="stylesheet"/> 
            <h1>Lighting Design</h1>
                <table className='theatrical-resume-table'>
                    <tr>
                        <th className='year-column'>2020</th>
                        <td className='show-column'>Erma Bombeck: At Wit's End *</td>
                        <td>Aurora Theatre</td>
                        <td className='supervisor-column'>Dir. Megan Rose-Houchins</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Little Shop of Horrors *</td>
                        <td>Clayton State University</td>
                        <td>Dir. Shontelle Thrash</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Five Course Love</td>
                        <td>Marietta Theatre Company</td>
                        <td>Dir. Stephanie Earle</td>
                    </tr>
                    <tr>
                        <th className='year-column'>2019</th>
                        <td>Zombie Prom</td>
                        <td>Georgia Gwinnett College</td>
                        <td>Dir. Renita James</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Erma Bombeck: At Wit's End</td>
                        <td>Aurora Theatre</td>
                        <td>Dir. Megan Rose-Houchins</td>
                    </tr>
                    <tr>
                        <th className='year-column'>2018</th>
                        <td>A Christams Carol</td>
                        <td>Heritage School</td>
                        <td>Dir. Candler Budd</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>The Addams Family</td>
                        <td>Georgia Gwinnett College</td>
                        <td>Dir. Jaclyn Hofmann</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Epic Proportions</td>
                        <td>Landmark Christian School</td>
                        <td>Dir. Emily Budd</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Willy Wonka and the Chocolate Factory Jr.</td>
                        <td>Aurora Theatre Summer Camp</td>
                        <td>Dir. Jaclyn Hofmann</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Grace, or The Art of Climbing</td>
                        <td>Aurora Theatre AppCo Alumni Series</td>
                        <td>Dir. Jasmine Renee Ellis</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Aurora AppCo 18 Showcase</td>
                        <td>Aurora Theatre</td>
                        <td>Dir. Jaclyn Hofmann</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Aurora Comedy Nights</td>
                        <td>Aurora Theatre Studio Series</td>
                        <td>PM Daniel Pope</td>
                    </tr>
                </table>
                <p>* Cancelled due to Covid-19</p>
            <h1>Assistant Lighting Design</h1>
                <table className='theatrical-resume-table'>
                    <tr>
                        <th className='year-column'>2019</th>
                        <td className='show-column'>Downstairs</td>
                        <td>Actor's Express</td>
                        <td className='supervisor-column'>LD Ben Rawson</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Native Gardens</td>
                        <td>Aurora Theatre</td>
                        <td>LD Ben Rawson</td>
                    </tr>
                    <tr>
                        <th className='year-column'>2018</th>
                        <td>Damn Yankees</td>
                        <td>Theatre Buford</td>
                        <td>LD Ben Rawson</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Cinderella: The Remix</td>
                        <td>Aurora Theatre</td>
                        <td>LD Kevin Frazier</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Maytag Virgin</td>
                        <td>Aurora Theatre</td>
                        <td>LD Kevin Frazier</td>
                    </tr>
                    <tr>
                        <th className='year-column'>2017</th>
                        <td>12 Dates of Christmas</td>
                        <td>Aurora Theatre Studio Series</td>
                        <td>LD Dylan Whitfield</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>A Christmas Carol</td>
                        <td>Aurora Theatre Studio Series</td>
                        <td>LD Dylan Whitfield</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>The 25th Annual Putnam Country Spelling Bee</td>
                        <td>Georgia Gwinnett College</td>
                        <td>LD Dylan Whitfield</td>
                    </tr>
                </table>
            <h1>Sound Design</h1>
                <table className='theatrical-resume-table'>
                        <tr>
                            <th className='year-column'>2019</th>
                            <td className='show-column'>A Streetcar Named Desire</td>
                            <td>Theatre Buford</td>
                            <td className='supervisor-column'>Dir. Daniel May</td>
                        </tr>
                        <tr>
                            <th className='year-column'>2018</th>
                            <td>A Christmas Carol</td>
                            <td>Theatre Buford</td>
                            <td>Dir Julie Skrzypek</td>
                        </tr>
                        <tr>
                            <th></th>
                            <td>Epic Proportions</td>
                            <td>Landmark Christian School</td>
                            <td>Dir. Emily Budd</td>
                        </tr>
                        <tr>
                            <th></th>
                            <td>The Electric Baby</td>
                            <td>Weird Sisters Theatre Project</td>
                            <td>Dir. Ibi Owolabi</td>
                        </tr>
                        <tr>
                            <th></th>
                            <td>Enchanted April</td>
                            <td>Weird Sisters Theatre Project</td>
                            <td>Dir. Kate Donadio MacQueen</td>
                        </tr>
                    </table>
            <h1>Lighting Programmer</h1>
                <table className='theatrical-resume-table'>
                    <tr>
                        <th className='year-column'>2020</th>
                        <td className='show-column'>On Your Feet</td>
                        <td>Aurora Theatre</td>
                        <td className='supervisor-column'>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Cat in the Hat</td>
                        <td>Aurora Theatre</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Curious Incident of the Dog in the Nighttime</td>
                        <td>Aurora Theatre</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th className='year-column'>2019</th>
                        <td>Urban Nutcracker</td>
                        <td>Ballethnic</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Winter Wonderland</td>
                        <td>Ballethnic</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th className='year-column'>2018</th>
                        <td>Urban Nutcracker</td>
                        <td>Ballethnic</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Winter Wonderland</td>
                        <td>Ballethnic</td>
                        <td>LD Mary Parker</td>
                    </tr>
                </table>
            <h1>Master Electrician</h1>
                <table className='theatrical-resume-table'>
                    <tr>
                        <th className='year-column'>2020</th>
                        <td className='show-column'>On Your Feet</td>
                        <td>Aurora Theatre</td>
                        <td className='supervisor-column'>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Cat in the Hat</td>
                        <td>Aurora Theatre</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Curious Incident of the Dog in the Nighttime</td>
                        <td>Aurora Theatre</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th className='year-column'>2019</th>
                        <td>Winter Wonderland</td>
                        <td>Ballethnic</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Urban Nutcracker</td>
                        <td>Ballethnic</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Ella Enchanted</td>
                        <td>Synchronicity Theatre</td>
                        <td>LD Dakota Adams</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Curious Incident of the Dog in the Nighttime</td>
                        <td>Horizon Theatre Company</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Rock the Presidents</td>
                        <td>Aurora Theatre</td>
                        <td>LD Mike Morin</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>The Roommate</td>
                        <td>Aurora Theatre</td>
                        <td>LD Mike Morin</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Sweet Water Taste</td>
                        <td>Horizon Theatre Company</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>The Cake</td>
                        <td>Horizon Theatre Company</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Native Gardens</td>
                        <td>Aurora Theatre</td>
                        <td>LD Ben Rawson</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Pipeline</td>
                        <td>Horizon Theatre Company</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Men With Money</td>
                        <td>Aurora Theatre</td>
                        <td>LD Mike Post</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>The Wolves</td>
                        <td>Horizon Theatre Company</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th className='year-column'>2018</th>
                        <td>Winter Wonderland</td>
                        <td>Ballethnic</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Urban Nutcracker</td>
                        <td>Ballethnic</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Nomad Motel</td>
                        <td>Horizon Theatre Company</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Newsies</td>
                        <td>Aurora Theatre</td>
                        <td>LD Maria-Cristina Fuste</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Ripcord</td>
                        <td>Aurora Theatre</td>
                        <td>LD Mary Parker</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Mamma Mia!</td>
                        <td>Aurora Theatre</td>
                        <td>LD Kevin Frazier</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Ontario Was Here</td>
                        <td>Aurora Theatre Studio Series</td>
                        <td>LD Maximo Grano de Oro</td>
                    </tr>
                    <tr>
                        <th></th>
                        <td>Throw Me on the Burnpile and Light Me Up</td>
                        <td>Aurora Theatre Studio Series</td>
                        <td>LD Maranda DeBusk</td>
                    </tr>
                </table>
        </div>
    )
};